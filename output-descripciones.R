...
dataset_descriptions <- sapply(names(working_data), function(dataset) {
  ...
  descriptions <- c(
    ...
    "INCAUTACIÓN DE COCAINA" = "❄️ Incautación de cocaína. Estas operaciones se realizan tanto en zonas rurales como urbanas e incluyen decomisos en rutas terrestres, marítimas y aéreas. La cocaína incautada suele ser destruida bajo supervisión judicial.",
...
dataset_legend_info <- sapply(names(working_data), function(dataset) {
  ...
  # Date Information
  date_info <- if ("fecha_hecho" %in% colnames(df)) {
    ...
    sprintf(
      "<br><strong>📅 Fechas:</strong> %s — %s <br><strong>⏳ Rango:</strong> %s", 
      min_date, max_date, time_range
    )
  } else {
    "<br><strong>📅 Fechas:</strong> No disponibles"
  }
  
  # Total Event Count
  event_info <- if ("fecha_hecho" %in% colnames(df)) {
    total_events <- sum(!is.na(df$fecha_hecho))
    
    sprintf("<br><strong>📊 Eventos totales:</strong> %s", 
            format(total_events, big.mark = ",", scientific = FALSE))
  } else {
    "<br><strong>📊 Eventos totales:</strong> No disponible"
  }
  
  # Unique Municipality Count
  mun_info <- if ("municipio" %in% colnames(df)) {
    unique_mun <- length(unique(na.omit(df$municipio)))
    
    sprintf("<br><strong>📍 Municipios:</strong> %s", 
            format(unique_mun, big.mark = ",", scientific = FALSE))
  } else {
    "<br><strong>📍 Municipios:</strong> No disponible"
  }
  # Measurement unit
  ...
  if ("cantidad" %in% colnames(df)) {
    ...
    cantidad_info <- "<br><strong>💫 Total intervenido:</strong> No disponible"
  }
