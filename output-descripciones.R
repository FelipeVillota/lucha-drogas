...
dataset_descriptions <- sapply(names(working_data), function(dataset) {
  ...
  if ("cantidad" %in% colnames(df)) {
    ...
    cantidad_info <- sprintf("<br><strong>Total:</strong> %s", total_cantidad)
  } else {
  ...
  descriptions <- c(
    ...
    "INCAUTACIÓN DE COCAINA" = "Estas operaciones se realizan tanto en zonas rurales como urbanas e incluyen decomisos en rutas terrestres, marítimas y aéreas. La cocaína incautada suele ser destruida bajo supervisión judicial.",
  ...
  paste0(descriptions[dataset], date_info, unidad_info, cantidad_info)
}, USE.NAMES = TRUE)
