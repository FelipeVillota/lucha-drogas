...
### Total CANTIDAD within the ranges of FECHA HECHO in each data frame
...
working_data[["INCAUTACIÓN DE COCAINA"]] <- 
  ...
  mutate(
    LATITUD = case_when(
      ...
      municipio == "MAR PACIFICO" ~ 0.0,
      municipio == "HOLANDA" ~ 52.1326,
      ...
      municipio == "AGUAS INTERNACIONALES" ~ NA_real_,
    ...
    LONGITUD = case_when(
      ...
      municipio == "EL SALVADOR" ~ -88.8965,
      municipio == "FRANCIA" ~ 1.8883,
      municipio == "BRASIL" ~ -51.9253,
      municipio == "ANTILLAS HOLANDESAS" ~ -69.0601,
    ...
    cod_muni = case_when(
      ...
      municipio == "VENEZUELA" ~ "862",        # Venezuela
      municipio == "ESTADOS UNIDOS" ~ "840",   # United States
      municipio == "ESTADOS UNIDOS DE AMERICA" ~ "840",  # Same as above
      municipio == "MEXICO" ~ "484",           # Mexico
      municipio == "ARGENTINA" ~ "032",        # Argentina
      municipio == "PANAMA" ~ "591",           # Panama
      municipio == "ESPAÑA" ~ "724",           # Spain
      municipio == "ITALIA" ~ "380",           # Italy
      municipio == "ALEMANIA" ~ "276",         # Germany
      municipio == "FRANCIA" ~ "250",          # France
      municipio == "REINO UNIDO" ~ "826",      # United Kingdom
      municipio == "CANADA" ~ "124",           # Canada
      municipio == "CHILE" ~ "152",            # Chile
      municipio == "CHINA" ~ "156",            # China
      municipio == "RUSIA" ~ "643",            # Russia
      municipio == "JAMAICA" ~ "388",          # Jamaica
      municipio == "COSTA RICA" ~ "188",       # Costa Rica
      municipio == "NICARAGUA" ~ "558",        # Nicaragua
      municipio == "GUATEMALA" ~ "320",        # Guatemala
      municipio == "HONDURAS" ~ "340",         # Honduras
      municipio == "EL SALVADOR" ~ "222",      # El Salvador
      municipio == "REPUBLICA DOMINICANA" ~ "214",  # Dominican Republic
      municipio == "PUERTO RICO" ~ "630",      # Puerto Rico (US territory)
      municipio == "CUBA" ~ "192",             # Cuba (LA HABANA)
      municipio == "URUGUAY" ~ "858",          # Uruguay
      municipio == "PARAGUAY" ~ "600",         # Paraguay
      municipio == "SUDAFRICA" ~ "710",        # South Africa
      municipio == "TURQUIA" ~ "792",          # Turkey
      municipio == "UCRANIA" ~ "804",          # Ukraine
      municipio == "PORTUGAL" ~ "620",         # Portugal
      municipio == "BELGICA" ~ "056",          # Belgium
      municipio == "HOLANDA" ~ "528",          # Netherlands (PAISES BAJOS)
      municipio == "PAISES BAJOS" ~ "528",     # Netherlands
      municipio == "ANTILLAS HOLANDESAS" ~ "530",  # Netherlands Antilles (dissolved in 2010)
      municipio == "SURINAM" ~ "740",          # Suriname
      municipio == "GUYANA" ~ "328",           # Guyana
      municipio == "GUINEA" ~ "324",           # Guinea
      municipio == "CABO VERDE" ~ "132",       # Cape Verde
      municipio == "LIBERIA" ~ "430",          # Liberia
      municipio == "MALTA" ~ "470",            # Malta
      municipio == "CROACIA" ~ "191",          # Croatia
      municipio == "RUMANIA" ~ "642",          # Romania
      municipio == "ALBANIA" ~ "008",          # Albania
      municipio == "MACEDONIA" ~ "807",        # North Macedonia
      municipio == "ARUBA" ~ "533",            # Aruba (Dutch territory)
      municipio == "CURAZAO" ~ "531",          # Curaçao (Dutch territory)
      municipio == "BAHAMAS" ~ "044",          # Bahamas
      municipio == "BELICE" ~ "084",           # Belize
      municipio == "MARTINICA" ~ "474",        # Martinique (French territory)
      
      # Non-Country Entries (Custom Codes)
      municipio == "AGUAS INTERNACIONALES" ~ "9999",  # International Waters
      municipio == "AGUAS JURISDICCIONALES" ~ "9998", # Jurisdictional Waters
      municipio == "MAR PACIFICO" ~ "9997",    # Pacific Ocean
      municipio == "MIAMI" ~ "9996",           # City in USA
      municipio == "MADRID" ~ "9995",          # City in Spain
      municipio == "AARHUS" ~ "9994",          # City in Denmark
      municipio == "DOVER" ~ "9993",           # City in UK
      municipio == "PUERTO VLISSINGEN" ~ "9992",  # City in Netherlands
      municipio == "LA HABANA" ~ "9991",       # Havana, Cuba
      
      TRUE ~ NA_character_  # Default for unmatched entries
    ), 
    cod_depto = case_when(
      # Use the same codes as cod_muni (adjust if departments are needed)
      municipio == "AGUAS INTERNACIONALES" ~ "9999",
      municipio == "AGUAS JURISDICCIONALES" ~ "9998",
      municipio == "MAR PACIFICO" ~ "9997",
      # ... (repeat all cod_muni entries here)
      TRUE ~ cod_muni  # Inherit cod_muni values by default
    )
  )
