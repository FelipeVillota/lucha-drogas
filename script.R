...
# dir.create("data/working_data")
saveRDS(working_data, "data/working_data/working_data.rds")
...
sink("sinks/tipos-cultivos-erradicados.txt")
table(working_data$ERRADICACIÓN$tipo_cultivo)
sink()
...
# Print the result
sink("sinks/resumen-totales-tiempo.txt")
...
sink("sinks/casos-departamentos.txt")
...
sink("sinks/casos-municipios.txt")
for (df_name in names(working_data)) {
  # Check if the dataframe has a "departamento" column
...
divipola <- read.csv("data/divipola.csv")
skim(divipola)
...
sink("sinks/municipios-por-departamento.txt")  # Open the sink to write to a file
divipola %>%
  group_by(NOM_DPTO) %>%
...
tableaver<- table(working_data$`INCAUTACIÓN DE COCAINA`$municipio) |> sort()
...
# Correciones CARTAGENA DE INDIAS 

working_data$`ASPERSION`$municipio <- gsub("^CARTAGENA$", "CARTAGENA DE INDIAS", working_data$`ASPERSION`$municipio)
working_data$`INCAUTACIÓN DE COCAINA`$municipio <- gsub("^CARTAGENA$", "CARTAGENA DE INDIAS", working_data$`INCAUTACIÓN DE COCAINA`$municipio)
working_data$`INCAUTACIÓN DE BASE DE COCA`$municipio <- gsub("^CARTAGENA$", "CARTAGENA DE INDIAS", working_data$`INCAUTACIÓN DE BASE DE COCA`$municipio)
working_data$`INCAUTACIÓN DE MARIHUANA`$municipio <- gsub("^CARTAGENA$", "CARTAGENA DE INDIAS", working_data$`INCAUTACIÓN DE MARIHUANA`$municipio)
working_data$`INCAUTACIÓN DE HEROINA`$municipio <- gsub("^CARTAGENA$", "CARTAGENA DE INDIAS", working_data$`INCAUTACIÓN DE HEROINA`$municipio)
